...
class MultiHeadAttention_classifier(nn.Module):
    def __init__(self, vocab, Model_session, n_layers=6, n_head=8,
                 d_word_vec=128, d_model=128, d_inner_hid=256, d_k=32, d_v=32,
                 dropout=0.1, proj_share_weight=True, embs_share_weight=True):
        super(MultiHeadAttention_classifier, self).__init__()
        vocab_size = len(vocab)
        max_seq_len = int(Model_session['fix_length'])
        label_size = int(Model_session['n_label'])

        self.encoder = Encoder(
            vocab_size, max_seq_len, n_layers=n_layers, n_head=n_head,
            d_word_vec=d_word_vec, d_model=d_model,
            d_inner_hid=d_inner_hid, dropout=dropout)
        self.hidden2label = nn.Linear(max_seq_len * d_model, label_size)
    ...
    def forward(self, seq, lengths=None):
        ...
        out = self.hidden2label(enc_output.view((batch_size, -1)))
        return out